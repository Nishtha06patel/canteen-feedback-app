...
const AdminForgotPassword = () => {
    const { admins, resetAdminPassword } = useAppContext();
            ...
            <div className="glass-panel auth-card animate-slide-up" style={{ width: '100%', padding: '2.5rem', textAlign: 'center' }}>
                ...
                {step === 2 && (
                    <form onSubmit={handleVerifyOTP} className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        {error && <p style={{ color: 'var(--danger)', fontSize: '0.875rem', marginTop: '0.5rem' }}>{error}</p>}
                        <button type="submit" className="btn btn-primary" style={{ marginTop: '1rem' }}>Verify Integrity</button>
                    </form>
                )}
                ...
                {step === 3 && (
                    <form onSubmit={handleSetNewPassword} className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <h1 style={{ marginBottom: '0.5rem', fontSize: '1.75rem' }}>Change Password</h1>
                        <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>Set your new localized password.</p>
                ...
                {step === 1 && (
                    <div style={{ marginTop: '2rem', borderTop: '1px solid var(--glass-border)', paddingTop: '1.5rem' }}>
                        <Link to="/admin/login" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.875rem', transition: 'color 0.2s' }}>
                            Remembered your password? <span style={{ color: 'var(--primary)' }}>Return &rarr;</span>
                        </Link>
                    </div>
                )}
            </div>
